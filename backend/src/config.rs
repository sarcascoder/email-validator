...
use email_validator_core::smtp::verif_method::{
...
use email_validator_core::{CheckEmailInputProxy, WebdriverConfig, LOG_TARGET};
use config::Config;
use lapin::Channel;
...
pub async fn load_config() -> Result<BackendConfig, anyhow::Error> {
	let cfg = Config::builder()
		...
		.add_source(config::Environment::with_prefix("EV").separator("__"));
	...
	warn!(target: LOG_TARGET, "The worker feature is currently in beta. Feedback welcome.");
...
mod tests {
	...
	async fn test_proxies() {
		env::set_var("EV__OVERRIDES__PROXIES__PROXY3__HOST", "test-proxy");
		env::set_var("EV__OVERRIDES__PROXIES__PROXY3__PORT", "1234");
		...
		env::remove_var("EV__OVERRIDES__PROXIES__PROXY3__HOST");
		env::remove_var("EV__OVERRIDES__PROXIES__PROXY3__PORT");
	}

	#[tokio::test]
	#[serial]
	async fn test_default_proxy() {
		env::set_var("EV__PROXY__HOST", "test-default-proxy");
		env::set_var("EV__PROXY__PORT", "5678");
		let cfg = load_config().await.unwrap();
		// Proxies
		assert_eq!(cfg.get_verif_method().proxies.len(), 1);
		assert_eq!(
		...
		env::remove_var("EV__PROXY__HOST");
		env::remove_var("EV__PROXY__PORT");
	}
...
"#;
	...
	async fn test_env_vars() {
		env::set_var("EV__BACKEND_NAME", "test-backend");
		env::set_var("EV__STORAGE__POSTGRES__DB_URL", "test2");
		let cfg = load_config().await.unwrap();
