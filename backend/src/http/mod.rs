...
use email_validator_core::LOG_TARGET;
use error::handle_rejection;
pub use error::ApiResponseError;
...
pub async fn run_warp_server(
	...
	let is_bulk_enabled = env::var("EV_ENABLE_BULK").unwrap_or_else(|_| "0".into()) == "1";
	let runner = if is_bulk_enabled {
		let pg_pool = config.get_pg_pool().expect(
			"Please set the EV__STORAGE__POSTGRES__DB_URL environment when EV_ENABLE_BULK is set",
		);
...
/// The header which holds the backend secret.
pub const EMAIL_VALIDATOR_SECRET_HEADER: &str = "x-email-validator-secret";
...
pub fn check_header(config: Arc<BackendConfig>) -> warp::filters::BoxedFilter<()> {
	if let Some(secret) = config.header_secret.clone() {
		...
		warp::header::exact(EMAIL_VALIDATOR_SECRET_HEADER, secret).boxed()
	} else {
		warp::any().boxed()
	}
}
