...
pub async fn create_job_registry(pool: &Pool<Postgres>) -> Result<JobRunnerHandle, sqlx::Error> {
    let min_task_conc = env::var("EV_MINIMUM_TASK_CONCURRENCY").map_or(10, |var| {
		var.parse::<usize>()
	    .expect("Environment variable EV_MINIMUM_TASK_CONCURRENCY should parse to usize")
	});
    let max_conc_task_fetch = env::var("EV_MAXIMUM_CONCURRENT_TASK_FETCH").map_or(20, |var| {
		var.parse::<usize>()
	    .expect("Environment variable EV_MAXIMUM_CONCURRENT_TASK_FETCH should parse to usize")
	});
