...
async fn job_result_as_iter(
	...
	offset: u64,
	pg_pool: PgPool,
) -> Result<Box<dyn Iterator<Item = serde_json::Value>>, ApiResponseError> {
...
async fn job_result_csv(
	...
	for json_value in rows {
		let result_csv: CsvResponse = CsvWrapper(json_value)
			.try_into()
			.map_err(|e| ApiResponseError::new(StatusCode::INTERNAL_SERVER_ERROR, e))?;
		wtr.serialize(result_csv)
			.map_err(|e| ApiResponseError::new(StatusCode::INTERNAL_SERVER_ERROR, e))?;
	}

	let data = wtr.into_inner().map_err(ApiResponseError::from)?;
...
pub fn v1_get_bulk_job_results(
	...
	warp::path!("v1" / "bulk" / i32 / "results")
		...
		// View access logs by setting `RUST_LOG=email_validator_backend`.
