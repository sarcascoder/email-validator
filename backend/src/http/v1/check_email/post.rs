...
use email_validator_core::{check_email, LOG_TARGET};
...
use crate::storage::commercial_license_trial::send_to_webhook;
...
async fn handle_without_worker(
	...
	let storage = Arc::clone(&config).get_storage_adapter();
		...
		// Optionally forward the result to a configured webhook.
		send_to_webhook(Arc::clone(&config), &body.to_email, &result_ok)
		.await
		.map_err(ApiResponseError::from)?;

	let result = result_ok.unwrap();
	info!(target: LOG_TARGET, email=body.to_email, is_reachable=?result.is_reachable, "Done verification");
	Ok(serde_json::to_vec(&result).map_err(ApiResponseError::from)?)
}
...
async fn handle_with_worker(
	...
	if let Some(delivery) = consumer.next().await {
		let delivery = delivery.map_err(ApiResponseError::from)?;
			...
			== Some(correlation_id.to_string().as_str())
		{
			...
			match single_shot_response {
				...
				SingleShotReply::Err((e, code)) => {
					let status_code =
						StatusCode::from_u16(code).map_err(ApiResponseError::from)?;
					return Err(ApiResponseError::new(status_code, e).into());
				}
			}
		} else {
			delivery
