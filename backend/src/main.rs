...
//! Main entry point of the `email_validator_backend` binary. It has two `main`
//! functions, depending on whether the `bulk` feature is enabled or not.

use email_validator_core::{setup_sentry, LOG_TARGET};
use email_validator_backend::config::load_config;
use email_validator_backend::http::run_warp_server;
use email_validator_backend::worker::run_worker;
use std::sync::Arc;
...
async fn main() -> Result<(), anyhow::Error> {
	...
	info!(target: LOG_TARGET, version=?CARGO_PKG_VERSION, "Running Email Validator");
