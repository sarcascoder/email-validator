...
use email_validator_core::{check_email, CheckEmailInput, CheckEmailOutput, Reachable, LOG_TARGET};
use http::HeaderMap;
...
pub(crate) async fn do_check_email_work(
	...
	match (&worker_output, delivery.redelivered) {
		...
		_ => {
			...
			// Optionally send result to an external webhook (if configured).
			send_to_webhook(config, &task.input.to_email, &worker_output).await?;
