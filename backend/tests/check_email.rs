...
mod tests {
	...
	use email_validator_backend::config::BackendConfig;
	use email_validator_backend::http::{create_routes, CheckEmailRequest, EMAIL_VALIDATOR_SECRET_HEADER};
	...
	async fn test_input_foo_bar() {
		let resp = request()
			.path("/v0/check_email")
			.method("POST")
			.header(EMAIL_VALIDATOR_SECRET_HEADER, "foobar")
	...
	#[tokio::test]
	async fn test_secret_missing_header() {
		let resp = request()
			.path("/v0/check_email")
			.method("POST")
			.json(
		...
		assert_eq!(resp.body(), r#"Missing request header \"x-email-validator-secret\""#);
	}

	#[tokio::test]
	async fn test_secret_wrong_secret() {
		let resp = request()
			.path("/v0/check_email")
			.method("POST")
			.header(EMAIL_VALIDATOR_SECRET_HEADER, "barbaz")
			.json(
				&serde_json::from_str::<CheckEmailRequest>(r#"{"to_email": "foo@bar.baz"}"#)
					.unwrap(),
			)
		...
		assert_eq!(resp.status(), StatusCode::BAD_REQUEST, "{:?}", resp.body());
	assert_eq!(resp.body(), r#"Invalid request header \"x-email-validator-secret\""#);
	}

	#[tokio::test]
	async fn test_secret_correct_secret() {
		let resp = request()
			.path("/v0/check_email")
			.method("POST")
			.header(EMAIL_VALIDATOR_SECRET_HEADER, "foobar")
