...
resource "aws_lambda_function" "lambda_task_check_email" {
  ...
  environment {
    variables = {
      RUST_LOG                = "email_validator=debug"
      EV__PROXY__HOST         = var.proxy_host
      EV__PROXY__PORT         = var.proxy_port
      EV__PROXY__USERNAME     = var.proxy_username
      EV__PROXY__PASSWORD     = var.proxy_password
      EV__FROM_EMAIL          = var.from_email
      EV__HELLO_NAME          = var.hello_name
      EV__WEBDRIVER__BINARY   = "/opt/chrome-linux64/chrome"
    }
  }
