...
use email_validator_core::CheckEmailOutput;
use email_validator_backend::config::{load_config, BackendConfig};
use email_validator_backend::http::CheckEmailRequest;
use email_validator_backend::storage::commercial_license_trial::send_to_webhook;
use email_validator_backend::worker::do_work::{
...
async fn main() -> Result<(), Error> {
	tracing_subscriber::fmt()
		...
		.with_target(false)
		.init();
	info!(version=?CARGO_PKG_VERSION, "Starting Email Validator SQS lambda.");
...
async fn handler(event: LambdaEvent<SQSPayload>) -> Result<CheckEmailOutput, Error> {
	...
	// TODO:
	// - Refactor storing the result and sending to webhook, it's duplicated
	...
	// If configured, also send the result to the webhook.
	send_to_webhook(backend_config, &task.input.to_email, &worker_output).await?;
